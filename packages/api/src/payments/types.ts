...
export const paySchema = z.object({
  ...
  /** Client-supplied idempotency key (recommended for card/paypay/wechat/alipay). */
  idempotencyKey: z.string().min(8).max(128).optional(),
  /** Demo-only: force simulator outcome when PAYMENT_MODE=sandbox without merchant keys. */
...
export type CreatePaymentResult = {
  provider: "mock" | "stripe" | "paypay" | "wechat" | "alipay";
  providerPaymentId: string;
  clientSecret?: string | null;
  /** QR / deeplink / code URL for customer scan */
...
export interface PaymentGateway {
  readonly name: "mock" | "stripe" | "paypay" | "wechat" | "alipay";
